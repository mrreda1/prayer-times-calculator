...
#define C_HA(x) ACOS((SIN(x) - SIN(LAT) * SIN(DELTA)) / (COS(LAT) * COS(DELTA)))
...
class PrayerApp {
  private:
	...
	double Y, M, D, B, A, T, JD, U, L0, TT, ET1000, ET, DELTA, SA_FAJR,
		SA_SUNRISE, SA_ASR, SA_MAGHRIB, SA_ISHA, FAJR, SUNRISE, ZUHR, ASR,
		MAGHRIB, ISHA, HA_FAJR, HA_SUNRISE, HA_ASR, HA_MAGHRIB, HA_ISHA;
	int time_in_minutes;
	prayers prayer[6];

  public:
	PrayerApp(LF Z, LF h, LF SF, LF LAT, LF LONG, LF FAJR_ANGLE,
			  LF ISHA_ANGLE) {
		H = 12, m = 0, s = 0;
	...
	void TimeEquation() {
		...
		ET1000 = -(1789 + 237 * U) * SIN(L0) - (7146 - 62 * U) * COS(L0) +
